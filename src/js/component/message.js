...
export default class MessageElement {
  ...
  render() {
    ...
    if (!this.isSatisfactionForm) {
      ...
      // warodri - start
      const isBot = this.message.customType == 'SENDBIRD_DESK_BOT_MESSAGE';
      console.log('isBot:' + isBot);
      if (isBot && this.message.data) {
        try {
          const data = JSON.parse(this.message.data);
          const result = data.results[ 0 ]; // Grab first even if there are many options
          let message  = result.answer.toString().replace(/(?:\\[rn]|[\r\n])/g, '<br/>');
          if (result.url) {
            message += result.url_label ? `<p><a href="${ result.url }" target="_blank">${ result.url_label }</a>` : `<a href="${ result.url }" target="_blank">${ result.url }</a></p>`;
          }
          ...
          this.media.hide();
          this.messageBox.show();  
          this.messageText.html('Unable to show this message.');
        }
      } else {
        // warodri - end
